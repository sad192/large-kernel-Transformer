...
class HAT_Net(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, dims=[64, 128, 256, 512],
        ...
        #     embed_dim=head_dim, norm_layer=nn.LayerNorm)
        ...
        self.patch_unembed = PatchUnEmbed(embed_dim=head_dim)
        # init weights
...
@register_model
# dims=[48, 96, 240, 384], head_dim=48
def HAT_Net_tiny(pretrained=False, **kwargs):
    model = HAT_Net(dims=[32, 64, 160, 256], head_dim=32, expansions=[8, 8, 4, 4],
        grid_sizes=[8, 7, 7, 1], ds_ratios=[8, 4, 2, 1], depths=[2, 2, 6, 3], **kwargs)
    model.default_cfg = {
        'num_classes': 1000, 'input_size': (3, 64, 64), 'pool_size': None,
