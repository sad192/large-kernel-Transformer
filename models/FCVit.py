...
class SpatialAtt(nn.Module):
    def __init__(self, dim, act_layer=nn.SiLU, params = params):
        super().__init__()
...
class ChannelAtt(nn.Module):
    def __init__(self, act_layer=nn.SiLU, params=params):
        super().__init__()
...
class ChannelMixer(nn.Module):
    def __init__(self, dim, hidden_dim=None, act_layer=nn.SiLU, drop=0., params=params):
        super().__init__()
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        B, HW, C = x.shape
        x = x.transpose(1, 2).reshape(B, self.embed_dim, x_size[0], x_size[1])  # B Ph*Pw C
        return x
...
class BaseFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('x:', x.shape) #(2,3,224,224)=>(2,32,56,56)
...
def fcvit_tiny(pretrained=False, **kwargs):
    ...
    layers = [3, 3, 5, 2]
    embed_dims = [32, 32, 32, 32] # [32, 64, 160, 320]
    mlp_ratios = [8, 8, 4, 4]
...
@register_model
def fcvit_block(pretrained=False, **kwargs):
