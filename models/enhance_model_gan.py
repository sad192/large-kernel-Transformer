...
from models.loss import GANLoss,compute_gradient_penalty
from models.loss_ssim import SSIMLoss
from utils.utils_image import tensor2img
from utils import youhuaGAN as util_gan
...
class ModelGAN(ModelBase):
    ...
    def __init__(self, opt):
        ...
        if len(opt["gpu_ids"]) > 1:  # 判断是不是有多个GPU
    ...
    def define_optimizer(self):
        ...
        if self.opt['train']['gan_type'] == 'wgan-gp':
            self.D_optimizer = RMSprop(self.netD.parameters(), lr=self.opt_train['D_optimizer_lr'], weight_decay=0)
            print('使用的是RMSprop优化器')
        else:
            self.D_optimizer = Adam(self.netD.parameters(), lr=self.opt_train['D_optimizer_lr'], weight_decay=0)
    ...
    def feed_data(self, data, need_H=True):
        # print('训练数据 img_lq：', data['L'])
        # print('训练数据 img_hq：', data['H'])
    ...
    def optimize_parameters(self, current_step):
        ...
        # 应用 minibatch discriminator
        # self.E = util_gan.miniBatchStdDev(self.E.detach())
        self.E = self.netG(self.L)  # 保存生成器G的输出
        pred_d_fake = self.netD(self.E.detach())  # 2) fake data, detach to avoid BP to G

        if current_step % 200:
            self.tensorboard_logger.add_scalar('pred_d_real_score', torch.mean(pred_d_real.detach()), current_step)
            self.tensorboard_logger.add_scalar('pred_d_fake_score', torch.mean(pred_d_fake.detach()), current_step)

        if self.opt['train']['gan_type'] in ['gan', 'lsgan', 'softplusgan']:
            l_d_real = self.D_lossfn(pred_d_real, True)
            l_d_fake = self.D_lossfn(pred_d_fake, False)
            loss_D_total = l_d_real + l_d_fake
        elif self.opt['train']['gan_type'] == 'hinge':
            l_d_real = torch.mean(1.0 - pred_d_real)
            l_d_fake = torch.mean(1.0 + pred_d_fake)  # 后续也可以尝试将1.0去掉
            loss_D_total = l_d_real + l_d_fake
        elif self.opt['train']['gan_type'] == 'wgan-gp':
            # Gradient penalty
            gradient_penalty = compute_gradient_penalty(self.netD, self.var_ref.data, self.E.data)
            # Adversarial loss
            loss_D_total = -torch.mean(pred_d_real) + torch.mean(pred_d_fake) + 10 * gradient_penalty
            ...
            # print('loss_D: ', loss_D_total)

        if current_step % 200:
            self.tensorboard_logger.add_scalar('D_all_loss', loss_D_total, current_step)
        ...
        if current_step % self.D_update_ratio == 0 and current_step > self.D_init_iters:  # updata D first
            ...
            if self.opt['train']['gan_type'] in ['gan', 'lsgan', 'softplusgan']:
                ...
                D_loss = -torch.mean(pred_g_fake)
                # print('G-gan_loss: ', D_loss)
            elif self.opt['train']['gan_type'] == 'wgan-gp':
                D_loss = -torch.mean(pred_g_fake)
            elif self.opt['train']['gan_type'] == 'ragan':
            ...
            if current_step % 200 :
                self.tensorboard_logger.add_scalar('G_all_loss', loss_G_total, current_step)
