...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        """
        Args:
            输入的张量X：numWindows*B, window_size * window_size, C
            num_windows = H*W / (window_size*window_size)，即窗口的个数
...
class SwinIR(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_first(x)
            # print('卷积后的大小：',x.shape)  # 180 120 200
            ...
            # 经过多个RSTB构建好的深度特征提取层后，x的形状未发生改变
            x = self.conv_after_body(self.forward_features(x)) + x  # 经过深层特征提取
