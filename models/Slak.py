...
from models.slak_depthwise import DepthWiseConv2dImplicitGEMM
...
class SLaK(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 layer_scale_init_value=1e-6, head_init_scale=1., kernel_size=[51, 49, 47, 13, 5], width_factor=1.0,
                 Decom=None, bn=True
                 ):
        super().__init__()
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(dim=dims[i], drop_path=dp_rates[cur + j],
                        ...
                        kernel_size=(self.kernel_size[i], self.kernel_size[-1]), Decom=Decom, bn=bn) for j in
                  range(depths[i])]
            )
...
if __name__ == '__main__':
    ...
    model = Block(Decom=True,kernel_size=(51,5),dim=3)
