...
class ModelBase():
    def __init__(self, opt):
        ...
        # 多GPU时在cuda:0 ,单GPU时在cuda:1
        self.device = torch.device('cuda:0' if opt['gpu_ids'] is not None else 'cpu')
        self.is_train = opt['is_train']        # training or not
        self.schedulers = []                   # schedulers
        self.G_optimizer = None
        self.base_lr_orig = 5e-4
    ...
    def update_learning_rate(self,current_epoch,need=True):
        if need:
            if current_epoch <= self.warmup_steps:
                increase = (self.base_lr_orig - self.warmup_begin_lr) \
                           * float(current_epoch) / float(self.warmup_steps)
                self.G_optimizer.param_groups[0]['lr'] = increase
            else:
    ...
    def current_learning_rate(self,current_epoch):
        if current_epoch <= 10:
