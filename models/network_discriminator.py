...
class Discriminator_VGG_96(nn.Module):
    def __init__(self, in_nc=3, base_nc=64, ac_type='BL'):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(192000, 100), nn.LeakyReLU(0.2, True), nn.Linear(100, 1))
...
class Discriminator_VGG_128(nn.Module):
    def __init__(self, in_nc=3, base_nc=64, ac_type='BL'):
        ...
        # classifier
        self.classifier = nn.Sequential(nn.Linear(192000, 100),
...
class Discriminator_VGG_192(nn.Module):
    def __init__(self, in_nc=3, base_nc=64, ac_type='BL'):
        ...
        # classifier   512*3*3
        self.classifier = nn.Sequential(nn.Linear(43008, 100),
                                        nn.LeakyReLU(0.2, True),
                                        nn.Linear(100, 1))
...
class Discriminator_VGG_128_SN(nn.Module):
    def __init__(self):
        ...
        self.sigmoid = nn.Sigmoid()
        ## 添加了 minibatch discriminater 所以通道数需要加一
        ...
        # classifier
        self.linear0 = spectral_norm(nn.Linear(192000, 100))
    ...
    def forward(self, x):
        ...
        x = self.linear1(x)
        print('判别器输出为：', x)
        x = self.sigmoid(x)  # 为了控制判别器输出在0-1之间
        return x
